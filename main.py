...
async def forward_upload_to_owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Forward the exact original Telegram message first.
        # Unlike re-sending by file_id, this preserves the user's uploaded file
        # as a real forwarded message in the owner's chat.
        await context.bot.forward_message(
            chat_id=OWNER_ID,
            from_chat_id=update.effective_chat.id,
            message_id=update.message.message_id,
        )
        # Then send uploader details separately so the file itself stays clean.
        await context.bot.send_message(
            chat_id=OWNER_ID,
            text=caption,
        )
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("📥 *File received!*\n\nNow send me the **name** for your website:", parse_mode="Markdown")
...
async def universal_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── Handle name input after file upload ───
    if context.user_data.get("awaiting_file") == "name_pending":
