...
def get_user_limit(uid):
    """Returns max sites allowed. -1 = unlimited, 0 = default(1), >0 = custom limit"""
    db = load_data()
    uid_str = str(uid)
    if uid == OWNER_ID:
        return -1  # Unlimited
    if uid_str in db.get("premium", []):
        return -1  # Premium = unlimited
    if uid_str in db.get("limits", {}):
        return db["limits"][uid_str]  # Custom limit
    return 1  # Default = 1
...
# ─── AUTO PING SYSTEM (Har 5 Min) ────────────────────────────
def auto_ping():
    """Prevent Render from sleeping by pinging every 5 minutes"""
    ...
    def ping_task():
        while True:
            ...
            try:
                # Ping bot's own URL
                if base_url:
                    http_requests.get(f"{base_url}/ping", timeout=10, verify=False)
                    logger.info(f"🔄 Auto-ping: {base_url}/ping")

                # Ping all deployed websites
                db = load_data()
                for sid, sinfo in db["sites"].items():
                    try:
                        http_requests.get(sinfo["url"], timeout=10, verify=False)
                        logger.info(f"🔄 Pinged site: {sinfo['url']}")
                    except:
                        pass
            except:
                pass
...
async def check_force_sub(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        await update.message.reply_text(
            f"❌ *Join Required!*\n\n"
            f"Please join @{channel} to use the bot.",
...
async def check_fsub_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channel = force_data.get("channel")
    if not channel:
        await query.edit_message_text("No force sub configured. Send /start")
        return
    try:
        member = await context.bot.get_chat_member(chat_id=f"@{channel}", user_id=user_id)
        if member.status in [ChatMember.MEMBER, ChatMember.ADMINISTRATOR, ChatMember.OWNER]:
        ...
        await query.edit_message_text("❌ Could not verify. Make sure bot is admin in channel.")
...
async def upload_website(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_upload(user.id):
        ...
        await update.message.reply_text(
            ...
            f"Delete a site first or contact admin for upgrade!",
            parse_mode="Markdown"
        )
        return
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    base_name = os.path.splitext(file_name)[0]  # For display name
...
async def my_websites(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Create inline buttons for each site
    buttons = []
...
async def my_websites_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("view_"):
        ...
        await query.edit_message_text("ℹ️ To re-deploy, delete this site first, then upload again using *📤 Upload Website* button.", parse_mode="Markdown")
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"2️⃣ Send `index.html` or `.zip`\n"
        f"3️⃣ Get live URL ✅\n\n"
        f"━━━ *Manage* ━━━\n"
        f"🌐 View & manage all your sites\n"
        f"🗑 Delete to upload new ones\n\n"
        f"━━━━━━━━━━━━━━━━━━━\n"
        f"📌 Default: 1 site\n"
        f"⭐ Premium: Unlimited\n"
        f"━━━━━━━━━━━━━━━━━━━",
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🎯 *Custom Limits:* `{len(db.get('limits',{}))}`\n"
        f"📢 *Force Sub:* {'✅ @' + channel if fsub_enabled else '❌ Off'}\n"
...
# ═════════════════════════════════════════════════════════════════
# 👑  A D M I N   P A N E L   (INLINE BUTTONS)
# ═════════════════════════════════════════════════════════════════

ADMIN_SET_LIMIT_USER, ADMIN_SET_LIMIT_VALUE = range(2)
...
async def admin_panel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"📢 Force Sub: {fsub_status} @{channel}"
    )
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_users":
        ...
        text = f"🌐 *Sites ({len(db['sites'])}):*\n\n"
        for sid, sinfo in db["sites"].items():
            text += f"├ `{sid[:12]}..` {sinfo.get('user_name','?')} → {sinfo['url']}\n"
        ...
        await query.message.edit_text(
            ...
            "Send me the channel username:\n\nExample: `@my_channel` or just `my_channel`",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", callback_data="admin_cancel")]])
        )
        ...
        await query.message.edit_text("✅ Force sub channel removed!")
        ...
        context.user_data["admin_action"] = None
        await query.message.edit_text("❌ Cancelled.", reply_markup=get_kb(user_id))
...
async def admin_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "ban":
        ...
        # Store user ID, ask for limit value
        context.user_data["setlimit_target"] = text
        context.user_data["admin_action"] = "setlimit_value"
        await update.message.reply_text(
            f"🎯 Target user: `{text}`\n\n"
            f"Now send the limit number (how many sites they can host):\n"
            f"• `2` = 2 sites\n"
            f"• `5` = 5 sites\n"
            f"• `0` = Reset to default (1)\n"
            f"• `-1` = Unlimited",
            parse_mode="Markdown",
        ...
        try:
            ...
            await update.message.reply_text(
                f"✅ *Limit Set!*\n\n"
                f"👤 User: `{target}`\n"
                f"🎯 Limit: {limit if limit >= 0 else 'Unlimited'} websites",
                parse_mode="Markdown",
                reply_markup=get_kb(OWNER_ID)
            )
        except ValueError:
...
def main():
    ...
    # Callback handlers
    app_tele.add_handler(CallbackQueryHandler(my_websites_callback, pattern=r'^(view_|del_|restart_|back_to_mywebsites|close_mywebsites|check_fsub)$'))
    app_tele.add_handler(CallbackQueryHandler(admin_callback_handler, pattern=r'^admin_|^fsub_'))

    # Button text handlers
    btn_pattern = r'^(📤 Upload Website|🌐 My Websites|❓ Help|📊 Status|⚙️ Admin Panel|❌ Cancel)$'
    ...
    app_tele.add_handler(MessageHandler(
        ...
        admin_text_handler,  # This runs for owner specifically
        group=-1
    ))

    # Fallback
    app_tele.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, fallback_text))
