...
# Native button styles were added to python-telegram-bot in v22.7.
try:
...
# Native Telegram button colors require python-telegram-bot >= 22.7.
# Bot API style field is supported by Telegram clients released after Feb 9, 2026.
...
def user_keyboard():
    return ReplyKeyboardMarkup([
        [KeyboardButton("📤 Upload Website", style="success")],
        [KeyboardButton("🌐 My Websites", style="primary"), KeyboardButton("❓ Help", style="primary")],
...
async def upload_website(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            [[KeyboardButton("❌ Cancel", style="danger")]], resize_keyboard=True
        )
    )
    ...
    if user.id == OWNER_ID and context.user_data.get("admin_action"):
        ...
        if action == "ban":
            ...
            await update.message.reply_text(
                ...
                f"• `2` = 2 sites\n• `5` = 5 sites\n• `0` = Default (1)\n• `-1` = Unlimited",
                parse_mode="Markdown",
                reply_markup=ReplyKeyboardMarkup([[KeyboardButton("❌ Cancel", style="danger")]], resize_keyboard=True)
            )
            return
