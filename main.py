import os, json, secrets, threading, logging, datetime, shutil, zipfile, io, time, re
...
# Native button styles are supported by python-telegram-bot 22.7+.
REQUIRED_PTB_VERSION = "22.7"

try:
    from telegram import __version__ as PTB_VERSION
    from packaging.version import Version
    if Version(PTB_VERSION) < Version(REQUIRED_PTB_VERSION):
        raise RuntimeError(
            f"python-telegram-bot>={REQUIRED_PTB_VERSION} is required for native colored buttons; "
            f"found {PTB_VERSION}. Update with: pip install -U python-telegram-bot>=22.7"
        )
except ImportError as exc:
    raise RuntimeError(
        "Missing python-telegram-bot/packaging. Install: "
        "pip install -U python-telegram-bot>=22.7 packaging"
    ) from exc

# ─── CONFIG ────────────────────────────────────────────────────
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8933123668:AAH-7TGHUSLOpjJJf8bM8pohd6Nz8Gow-Ko")
...
def user_keyboard():
    return ReplyKeyboardMarkup([
        [KeyboardButton(text="📤 Upload Website", style="success")],
        [KeyboardButton(text="🌐 My Websites", style="primary"), KeyboardButton(text="❓ Help", style="primary")],
...
def owner_keyboard():
    return ReplyKeyboardMarkup([
        ...
        [KeyboardButton(text="🌐 My Websites", style="primary"), KeyboardButton(text="❓ Help", style="primary")],
        [KeyboardButton(text="📊 Status", style="primary")],
        [KeyboardButton(text="⚙️ Admin Panel", style="danger")]
    ], resize_keyboard=True)
...
@app_flask.route("/")
def home():
    # Show list of all live sites
    db = load_data()
    site_list = ""
    for sid, sinfo in db["sites"].items():
        if os.path.exists(os.path.join(SITES_DIR, sid, "index.html")):
            site_list += f'<li><a href="/{sid}">{sid}</a> — {sinfo.get("user_name","?")}</li>\n'
    if not site_list:
        site_list = "<li>No sites deployed yet</li>"
    return f"""
    <html>
    <head><meta charset="utf-8"><title>Website Hoster</title>
    <style>body{{font-family:sans-serif;max-width:700px;margin:40px auto;padding:0 20px}}
    h2{{color:#2ecc71}} ul{{list-style:none;padding:0}} li{{padding:8px 0}}
    a{{color:#3498db;text-decoration:none}} a:hover{{text-decoration:underline}}
    </style></head>
    <body>
    <h2>🤖 Website Hoster Bot — Online ✅</h2>
...
def serve_site(site_id):
    ...
    if not os.path.isdir(site_path):
        ...
        <h2 style="color:#e74c3c">❌ Site Not Found</h2>
        <p>No site with ID <code>{site_id}</code> exists.</p>
    ...
    # Try index.htm
    index_htm = os.path.join(site_path, "index.htm")
    if os.path.exists(index_htm):
        return send_from_directory(site_path, "index.htm")

    # Fallback: find any .html file in the directory
    html_files = [f for f in os.listdir(site_path) if f.endswith(('.html', '.htm'))]
    if html_files:
        first_file = html_files[0]
        return send_from_directory(site_path, first_file)

    # No HTML files at all — list contents for debugging
    files = os.listdir(site_path)
    file_list = "<br>".join(files) if files else "Directory is empty"
    return f"""
    <html><body style="font-family:sans-serif;text-align:center;padding-top:40px">
    <h2 style="color:#e67e22">⚠️ No HTML Found</h2>
    <p>Files in this site directory:</p>
    <code>{file_list}</code>
    <p><a href="/">← Back to Home</a></p>
    </body></html>
    """, 200
...
async def check_force_sub(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        ...
        btn = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔗 Join Channel", style="primary", url=f"https://t.me/{channel}")],
            [InlineKeyboardButton("✅ Joined", style="success", callback_data="check_fsub")]
        ])
...
async def upload_website(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            [[KeyboardButton(text="❌ Cancel", style="danger")]], resize_keyboard=True
        )
    )


# ─── Owner Upload Forwarder ─────────────────────────────────────
async def forward_upload_to_owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Forward the original uploaded document to the owner."""
    ...
    try:
        try:
            # First try a real forward of the exact uploaded Telegram message.
            ...
            # Fallback: send the same Telegram file by file_id.
        ...
        logger.exception("Failed to forward uploaded file to owner")
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not (fname_lower.endswith(".zip") or fname_lower.endswith((".html", ".htm"))):
        await update.message.reply_text("❌ Only `.html` or `.zip` files allowed!")
        context.user_data["awaiting_file"] = False
        return

    # Forward a copy of the original file to the owner.
    await forward_upload_to_owner(update, context)

    if not can_upload(user.id):
        await update.message.reply_text("❌ Limit reached! Delete old site first.")
        context.user_data["awaiting_file"] = False
        return

    # Mark name-entry state before the potentially slow download.
    context.user_data["awaiting_file"] = "name_pending"

    # Download file into memory
    msg = await update.message.reply_text("📥 *File received!*\n\nNow send me the **name** for your website:\n\nExample: `my-site`", parse_mode="Markdown")
    ...
    # Store in context for the next message
...
# ─── Deploy with User-Provided Name (FIXED) ──────────────────
async def deploy_with_name(update: Update, context: ContextTypes.DEFAULT_TYPE, name: str):
    """Deploy the stored file using the user-provided name."""
    user = update.effective_user
    uid = str(user.id)
    pending = context.user_data.get("pending_file")

    if not pending:
        await update.message.reply_text("❌ Something went wrong. Please start over.", reply_markup=get_kb(user.id))
        return

    file_bytes = pending["file_bytes"]
    file_name = pending["file_name"]
    fname_lower = pending["fname_lower"]

    # Sanitize name: only alphanumeric, hyphens, underscores
    site_id = re.sub(r'[^a-zA-Z0-9_-]', '', name.strip().replace(' ', '-'))
    if not site_id:
        site_id = "website"

    # Deduplication: if name exists, append number (1, 2, 3...)
    original_site_id = site_id
    counter = 1
    while os.path.exists(os.path.join(SITES_DIR, site_id)):
        site_id = f"{original_site_id}{counter}"
        counter += 1

    msg = await update.message.reply_text(f"⏳ Deploying as `{site_id}`...", parse_mode="Markdown")

    try:
        site_dir = os.path.join(SITES_DIR, site_id)
        os.makedirs(site_dir, exist_ok=True)
        logger.info(f"Created directory: {site_dir}")
            ...
            try:
                with zipfile.ZipFile(file_bytes, 'r') as zf:
                    if zf.testzip():
                        ...
                        return
                    # Extract everything preserving directory structure
                    zf.extractall(site_dir)
                    
                    # Check if index.html exists at root or nested
                    all_files = []
                    ...
                    logger.info(f"ZIP extracted. Files: {all_files}")
                    
                    has_index = False
                    for f in all_files:
                        if f == "index.html" or f.endswith("/index.html") or os.path.basename(f) == "index.html":
                            has_index = True
                            # If index.html is in a subfolder, move everything up
                            if os.path.dirname(f):
                                sub_dir = os.path.dirname(f)
                                for item in os.listdir(os.path.join(site_dir, sub_dir)):
                                    shutil.move(os.path.join(site_dir, sub_dir, item), os.path.join(site_dir, item))
                                shutil.rmtree(os.path.join(site_dir, sub_dir))
                            break
                    
                    if not has_index:
                        # Try to find any .html to rename as index.html
                        html_in_zip = [f for f in all_files if f.endswith(('.html', '.htm'))]
                        if html_in_zip:
                            first_html = html_in_zip[0]
                            src = os.path.join(site_dir, first_html)
                            # If it's in a subfolder, move to root
                            if os.path.dirname(first_html):
                                sub_dir = os.path.dirname(first_html)
                                for item in os.listdir(os.path.join(site_dir, sub_dir)):
                                    shutil.move(os.path.join(site_dir, sub_dir, item), os.path.join(site_dir, item))
                                shutil.rmtree(os.path.join(site_dir, sub_dir))
                            # Rename to index.html
                            for f in os.listdir(site_dir):
                                if f.endswith(('.html', '.htm')) and f != "index.html":
                                    os.rename(os.path.join(site_dir, f), os.path.join(site_dir, "index.html"))
                                    break
                            has_index = True
                    
                    if not has_index:
                        await msg.edit_text("❌ ZIP must contain `index.html` or an HTML file!")
        ...
        # Verify index.html exists in site_dir
        index_final = os.path.join(site_dir, "index.html")
        if not os.path.exists(index_final):
            # Try to find any html file and rename it
            html_files_final = [f for f in os.listdir(site_dir) if f.endswith(('.html', '.htm'))]
            if html_files_final:
                os.rename(os.path.join(site_dir, html_files_final[0]), index_final)
                logger.info(f"Renamed {html_files_final[0]} to index.html")

        # Double-check
        if not os.path.exists(index_final):
            await msg.edit_text("❌ Failed to create index.html!")
            shutil.rmtree(site_dir)
            return
        ...
        db = load_data()
        db["sites"][site_id] = {
            "user_id": uid, "url": site_url, "name": site_id,
        ...
        logger.info(f"✅ Deployed site '{site_id}' for user {user.id} at {site_url}")
        ...
        inline_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🌐 Open Website", style="success", url=site_url)],
            [InlineKeyboardButton("🗑 Delete", style="danger", callback_data=f"del_{site_id}"),
             InlineKeyboardButton("🔄 Re-deploy", style="primary", callback_data=f"restart_{site_id}")]
        ])
        ...
        logger.exception(f"Deploy error for name '{name}': {e}")
        # Clean up on failure
        site_dir = os.path.join(SITES_DIR, site_id)
...
async def my_websites(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for sid, sinfo in user_sites.items():
        ...
        buttons.append([InlineKeyboardButton(f"🌐 {name}", style="primary", callback_data=f"view_{sid}")])
    buttons.append([InlineKeyboardButton("❌ Close", style="danger", callback_data="close_mywebsites")])
...
async def my_websites_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("view_"):
        ...
        if site_id in db["sites"]:
            ...
            inline_kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("🌐 Open Website", style="success", url=sinfo["url"])],
                [InlineKeyboardButton("🗑 Delete", style="danger", callback_data=f"del_{site_id}"),
                 InlineKeyboardButton("🔄 Re-deploy", style="primary", callback_data=f"restart_{site_id}")],
                [InlineKeyboardButton("🔙 Back", style="primary", callback_data="back_to_mywebsites")]
            ])
...
async def admin_panel_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [InlineKeyboardButton("👥 Users", style="primary", callback_data="admin_users"),
         InlineKeyboardButton("🌐 Sites", style="primary", callback_data="admin_sites")],
        [InlineKeyboardButton("🚫 Ban User", style="danger", callback_data="admin_ban"),
         InlineKeyboardButton("✅ Unban User", style="success", callback_data="admin_unban")],
        [InlineKeyboardButton("⭐ Make Premium", style="success", callback_data="admin_premium"),
         InlineKeyboardButton("💔 Un-Premium", style="danger", callback_data="admin_unpremium")],
        [InlineKeyboardButton("🎯 Set Limit", style="primary", callback_data="admin_setlimit")],
        [InlineKeyboardButton("📢 Force Sub Setup", style="primary", callback_data="admin_fsub")],
        [InlineKeyboardButton("📤 Broadcast", style="primary", callback_data="admin_broadcast"),
         InlineKeyboardButton("📊 Full Stats", style="primary", callback_data="admin_fullstats")],
        [InlineKeyboardButton("❌ Close", style="danger", callback_data="admin_close")]
    ]
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_users":
        ...
        for sid, sinfo in db["sites"].items():
            ...
            buttons.append([InlineKeyboardButton(f"🌐 {name} — {uname}", style="primary", callback_data=f"admin_viewsite_{sid}")])
        buttons.append([InlineKeyboardButton("🔙 Back", style="primary", callback_data="admin_back")])
        await query.edit_message_text(
        ...
        if site_id in db["sites"]:
            ...
            inline_kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("🌐 Open Website", style="success", url=sinfo["url"])],
                [InlineKeyboardButton("🗑 Delete", style="danger", callback_data=f"admin_del_{site_id}"),
                 InlineKeyboardButton("🔄 Re-deploy", style="primary", callback_data=f"restart_{site_id}")],
                [InlineKeyboardButton("🔙 Back to Sites", style="primary", callback_data="admin_sites")]
            ])
            await query.edit_message_text(
                f"🌐 *{name}*\n\n"
        ...
        await query.edit_message_text(
            ...
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel", style="danger", callback_data="admin_cancel")]])
        )
        ...
        buttons = [
            [InlineKeyboardButton("📢 Set Channel", style="primary", callback_data="fsub_set")],
            [InlineKeyboardButton("🗑 Remove Channel", style="danger", callback_data="fsub_remove")],
            [InlineKeyboardButton("🔙 Back", style="primary", callback_data="admin_back")]
        ]
        await query.edit_message_text(
...
async def universal_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── Handle website-name input after file upload ───
    # pending_file is the authoritative state. This prevents the name from
    # falling through to the generic "Use buttons below" handler.
    pending_file = context.user_data.get("pending_file")
    if pending_file is not None:
        button_texts = {
            "📤 Upload Website", "🌐 My Websites", "❓ Help",
            "📊 Status", "⚙️ Admin Panel", "❌ Cancel"
        }
        ...
        if text in button_texts:
            if text == "❌ Cancel":
                return await cancel_handler(update, context)
            await update.message.reply_text(
                "📝 Please send the *website name* first.\n\n"
                "Example: `my-site`",
                parse_mode="Markdown"
            )
            return

        return await deploy_with_name(update, context, text)
    ...
    if user.id == OWNER_ID and context.user_data.get("admin_action"):
        ...
        if action == "ban":
            ...
            await update.message.reply_text(
                ...
                f"• `2` = 2 sites\n• `5` = 5 sites\n• `0` = Default (1)\n• `-1` = Unlimited",
                parse_mode="Markdown",
                reply_markup=ReplyKeyboardMarkup([[KeyboardButton(text="❌ Cancel", style="danger")]], resize_keyboard=True)
            )
            return
